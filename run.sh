...
run_vgl() {
  ...
  docker run --rm -v "$final_dir":/app/data ghcr.io/josehinojosahidalgo/jetson_vgl:1.0 poetry run python /app/scripts/main.py
}
...
run_odm()  {
  ...
  if [[ "$response" == "y" || "$response" == "yes" ]]; then
    ...
    docker run -ti --rm -v "$(dirname "$2")":/datasets --gpus all opendronemap/odm:gpu --project-path /datasets "$(basename "$2")" --orthophoto-resolution="$1" --fast-orthophoto --skip-band-alignment --skip-report
  elif [[ "$response" == "n" || "$response" == "no" ]]; then
    echo "Running ODM in $DIR1 cpu..."
    docker run -ti --rm -v "$(dirname "$2")":/datasets opendronemap/odm --project-path /datasets "$(basename "$2")" --orthophoto-resolution="$1" --fast-orthophoto --skip-band-alignment --skip-report
  else
    echo ""
    echo "Invalid input. Please answer 'y' or 'n'."
    exit 1 # Exit with an error code for invalid input
